...
@pytest.fixture
def workspace_wiegendrucke(workspace):
    # http://digital.slub-dresden.de/id312439970
    yield from workspace("https://digital.slub-dresden.de/oai/?verb=GetRecord&metadataPrefix=mets&identifier=oai:de:slub-dresden:db:id-312439970")

@pytest.fixture
def workspace_mundkoch(workspace):
    # http://digital.slub-dresden.de/id1832552268
    yield from workspace("https://digital.slub-dresden.de/oai/?verb=GetRecord&metadataPrefix=mets&identifier=oai:de:slub-dresden:db:id-1832552268")

@pytest.fixture
def workspace_boersenblatt(workspace):
    # http://digital.slub-dresden.de/id39946221X-18440511
    yield from workspace("https://digital.slub-dresden.de/oai/?verb=GetRecord&metadataPrefix=mets&identifier=oai:de:slub-dresden:db:id-39946221X-18440511")

@pytest.fixture
def workspace_homercomment(workspace):
    # http://dx.doi.org/10.25673/82290
    yield from workspace("https://opendata.uni-halle.de/explore?bitstream_id=1854f8cb-6561-4faf-bee3-a3d4c65ff2fd&handle=1981185920/84245&provider=dfg-viewer&onlyManifest=true")

@pytest.fixture
def workspace_pembroke(workspace):
    # http://resolver.staatsbibliothek-berlin.de/SBB0001CA7900000000
    yield from workspace("https://content.staatsbibliothek-berlin.de/dc/PPN85249078X.mets.xml")
